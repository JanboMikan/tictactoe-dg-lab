...
- ✅ 设备断开连接处理：APP主动断开时自动通知前端更新设备状态
- ✅ 前端 Toast 通知：震动事件和设备连接/断开实时显示在前端界面
...
2. **hub.go** - 连接管理中心
   ...
   - `OnBindSuccess`: 设备绑定成功回调（通知游戏 Hub 更新状态）
   - `OnDeviceDisconnect`: 设备断开连接回调（通知游戏 Hub 更新状态）
   - **并发安全**: 使用 `sync.RWMutex` 保护共享数据
   - **断开处理**: 自动识别控制端/APP端断开，清理绑定关系，通知伙伴
